    ...
    <>
      <header className={cn(
        ...
        <div className={cn(
          ...
          scrolled 
            ? "w-[95%] rounded-full bg-white/90 px-4 h-14 md:h-16" 
            : "w-[95%] rounded-2xl md:rounded-3xl bg-white/80 px-4 md:px-8 h-16 md:h-20"
        )}>
          
          {/* Logo */}
          <Link href="/" className="flex items-center gap-3 group">
            <div className="h-8 w-8 md:h-10 md:w-10 bg-gradient-to-br from-cobalt to-stone-800 rounded-lg md:rounded-xl flex items-center justify-center text-white shadow-lg shadow-cobalt/30 group-hover:scale-105 transition-transform rotate-0 group-hover:rotate-180 duration-500">
              <Hexagon className="h-5 w-5 md:h-6 md:w-6 fill-white/20" strokeWidth={2.5} />
            </div>
            <div className="hidden md:flex flex-col">
              <span className="font-display font-bold text-lg leading-none tracking-tight text-ink">MAG<span className="font-light text-stone-500">GROUP</span></span>
            </div>
          </Link>
        ...
        <div className="fixed inset-0 z-40 bg-white/95 backdrop-blur-3xl p-6 flex flex-col pt-24 animate-in fade-in slide-in-from-top-10">
           <div className="flex flex-col gap-3">
             ...
             <Link href="/contact" onClick={() => setIsMenuOpen(false)} className="mt-4">
                <Button className="w-full h-16 text-lg rounded-2xl shadow-xl bg-gradient-to-r from-ink to-stone-800">Start Project</Button>
             </Link>
           </div>
        </div>
      )}
