  ...
  return (
    <div className="relative pt-24 md:pt-32 pb-8 md:pb-12 px-4 md:px-8 min-h-[90vh] md:min-h-screen w-full bg-paper flex flex-col justify-center overflow-hidden">
      ...
      <div className="max-w-[1800px] mx-auto w-full flex flex-col lg:flex-row gap-4 md:gap-6 relative z-10 flex-grow lg:h-[80vh]">
        ...
        {/* LEFT: Info Card */}
        <div className="w-full lg:w-[40%] bg-glass backdrop-blur-xl rounded-[2rem] md:rounded-[3rem] p-6 md:p-12 lg:p-16 shadow-2xl shadow-stone-900/5 border border-white/60 relative overflow-hidden z-20 flex flex-col justify-center min-h-[400px]">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="inline-flex items-center gap-2 bg-gradient-to-r from-amber-50 to-yellow-50 rounded-full px-3 py-1 md:px-4 md:py-1.5 mb-6 md:mb-8 w-fit border border-amber-100">
              ...
              <div className="flex flex-wrap gap-4 items-center">
                 <Link href="/products">
                    <Button className="h-12 md:h-14 rounded-full bg-ink text-white hover:bg-cobalt px-6 md:px-8 shadow-xl shadow-stone-300 text-xs md:text-sm">
                 ...
                 <div className="flex items-center gap-2 px-2 md:px-4">
                    {slides.map((_, i) => (
                      <button 
                        ...
                        className={`w-2 h-2 md:w-3 md:h-3 rounded-full transition-all duration-300 ${current === i ? 'bg-cobalt scale-125 ring-2 md:ring-4 ring-amber-100' : 'bg-stone-300 hover:bg-stone-400'}`}
                      />
                    ))}
                 </div>
              </div>
        ...
        {/* RIGHT: Image Card */}
        <div className="w-full lg:w-[60%] relative bg-stone-200 rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-2xl shadow-ink/10 group border-4 border-white h-[300px] lg:h-auto">
           <AnimatePresence mode="wait">
              <MotionDiv
                key={current}
           ...
           <div className="absolute top-4 right-4 md:top-8 md:right-8 bg-white/90 backdrop-blur-md rounded-full px-4 py-2 md:px-6 md:py-3 shadow-lg flex items-center gap-2 md:gap-3 border border-white/50">
              <div className="bg-yellow-500 w-1.5 h-1.5 md:w-2 md:h-2 rounded-full animate-pulse" />
