         ...
         >
            {badge && (
              <span className={cn(
                ...
                dark ? "bg-white/10 border-white/20 text-yellow-300" : "bg-cobalt/10 border-cobalt/20 text-cobalt"
              )}>
                  {badge}
              </span>
            )}
            <h1 className="text-4xl sm:text-5xl md:text-8xl font-display font-bold uppercase mb-4 md:mb-6 leading-[0.9] tracking-tighter drop-shadow-2xl">
                {title.split(' ').map((word, i) => (
                  <span key={i} className={i % 2 !== 0 ? "text-transparent bg-clip-text bg-gradient-to-b from-white to-stone-400" : ""}>{word} </span>
                ))}
            </h1>
            <p className={cn(
              "text-base md:text-xl max-w-2xl mx-auto font-medium leading-relaxed",
              dark ? "text-stone-300" : "text-tech-500"
            )}>
              {subtitle}
            </p>
         </motion.div>
